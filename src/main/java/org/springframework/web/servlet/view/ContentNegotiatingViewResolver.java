...
public class ContentNegotiatingViewResolver extends WebApplicationObjectSupport
	...
	private View getBestView(List<View> candidateViews, List<MediaType> requestedMediaTypes, RequestAttributes attrs) {
		List<View> candidateSmartViews = new LinkedList<>();
		for (View candidateView : candidateViews) {
			if (candidateView instanceof SmartView) {
				SmartView smartView = (SmartView) candidateView;
				if (smartView.isRedirectView()) {
					// return candidateView;
					candidateSmartViews.add(smartView);
				}
			}
		}
		List<View> candidateViewsOrCandidateSmartViews = new LinkedList<>();
		if (candidateSmartViews.isEmpty()) {
			candidateViewsOrCandidateSmartViews.addAll(candidateViews);
		}
		else {
			candidateViewsOrCandidateSmartViews.addAll(candidateSmartViews);
		}
		for (MediaType mediaType : requestedMediaTypes) {
			for (View candidateView : candidateViewsOrCandidateSmartViews) {
